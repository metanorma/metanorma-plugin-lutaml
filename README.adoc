...
This command allows to render a LutaML GML Dictionary.

[source,adoc]
-----
lutaml_gml_dictionary::[xml_path="/path/to/dictionary.xml",source="source_string",template_path="/path/to/template.liquid"]
-----

The command accepts the options listed below:

* `xml_path="/path/to/dictionary.xml` specifies the path of xml file of the
GML Dictionary.

* `source="source_string"` shows the link of the anchor.

* `template_path="/path/yo/template.liquid"` specifies the liquid template.
  For example, you can create a liquid template shown below and link it to
  `template_path`.
